...
// TODO: These don't appear to work on Clang
// #ifdef _MSC_VER
...
#define IF_LOG_(severity)                                         \
	!(Logging::Logger::GetSingleton() &&                          \
	  Logging::Logger::GetSingleton()->checkSeverity(severity)) ? \
	    (void)0:
...
#define LOG_(severity)                    \
	IF_LOG_(severity)                     \
	(*Logging::Logger::GetSingleton()) += \
	    Logging::Record(severity, LOGGER_GET_FUNC(), __LINE__, LOGGER_GET_FILE())
#define LOG(severity) LOG_(severity)

#define LOG_VERBOSE LOG(Logging::Severity::verbose)
#define LOG_DEBUG LOG(Logging::Severity::debug)
#define LOG_INFO LOG(Logging::Severity::info)
#define LOG_WARNING LOG(Logging::Severity::warning)
#define LOG_ERROR LOG(Logging::Severity::error)
#define LOG_FATAL LOG(Logging::Severity::fatal)
...
#define LOG_VERBOSE_IF(condition) LOG_IF(Logging::Severity::verbose, condition)
#define LOG_DEBUG_IF(condition) LOG_IF(Logging::Severity::debug, condition)
#define LOG_INFO_IF(condition) LOG_IF(Logging::Severity::info, condition)
#define LOG_WARNING_IF(condition) LOG_IF(Logging::Severity::warning, condition)
#define LOG_ERROR_IF(condition) LOG_IF(Logging::Severity::error, condition)
#define LOG_FATAL_IF(condition) LOG_IF(Logging::Severity::fatal, condition)
...
// Only output function with message
